...
<div class="contain-item">
  ...
  <el-table :data="tableData" border stripe @cell-dblclick="dbclick" :row-key="tableData.index" height="100%">
    ...
    <el-table-column prop="name" column-key="name" label="分类名称" >
      <template #default="scope">
        <el-input v-if="current === scope.row.id" v-model.trim="typeName" :maxlength="10" :autofocus="true">
          <template #append>
  ...
  methods:{
    ...
    add(value){
      if (0 < value.length < 10){
        request.post(AdminApi.TYPE+'/'+ value).then(res=>{
          if (res.code===2000){
            this.reloadData()
          }
        })
    ...
    remove(id){
      request.delete(AdminApi.TYPE+"/" +id).then(res=>{
        if (res.code === 2000){
          this.reloadData()
        }
      })
